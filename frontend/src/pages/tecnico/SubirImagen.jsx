...
function TecnicoSubirImagen() {
  ...
  const handleImagen = (e) => {
    ...
    if (!file) return
    if (!['image/jpeg', 'image/png'].includes(file.type)) { setErrorMsg('Solo se aceptan imágenes JPG o PNG.'); return }
    if (file.size > 10 * 1024 * 1024) { setErrorMsg('La imagen no puede superar 10MB.'); return }
    const img = new Image()
    img.onload = () => {
      if (img.width < 224 || img.height < 224) { setErrorMsg(`Imagen muy pequeña (${img.width}x${img.height}px). Mínimo 224x224px.`); return }
      setErrorMsg('')
      setImagen(file)
  ...
  const handleSubir = async () => {
    ...
    // Notificar solo al médico seleccionado — sin duplicados
  ...
  if (!paciente) return <div className="flex items-center justify-center h-screen"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-teal-600" /></div>
          ...
          <div onClick={() => fileRef.current.click()} className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-teal-400 hover:bg-teal-50 transition">
            {preview ? (
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">JPG / PNG — mín. 224x224px — máx. 10MB</p>
              </div>
            )}
          </div>
          <input ref={fileRef} type="file" accept="image/jpeg,image/png" onChange={handleImagen} className="hidden" />
