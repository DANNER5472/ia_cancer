...
function NuevoPaciente() {
  ...
  useEffect(() => {
    supabase.from('profiles').select('id, nombre').eq('rol', 'tecnico').eq('activo', true)
      .then(({ data }) => {
        setTecnicos(data ?? [])
        if (data?.length === 1) setTecnicoSeleccionado(data[0].id)
      })
  }, [])

  const handleTipoCancer = (tipo) => {
    setConsulta({...consulta, tipo_cancer_analizar: tipo})
    if (tipo === 'cervical') setIdentificacion(prev => ({...prev, sexo: 'F'}))
  }
  ...
  const handleFinalizar = async () => {
    ...
    setTimeout(() => { window.location.href = '/enfermera/pacientes' }, 2000)
  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {consulta.tipo_cancer_analizar === 'cervical' ? (
                    <div className="mt-1 px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 text-gray-500 text-sm">Femenino (requerido para cáncer cervical)</div>
                  ) : (
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select value={consulta.tipo_cancer_analizar} onChange={e => handleTipoCancer(e.target.value)} className={inputClass('')}>
                    <option value="cervical">Cervical (Papanicolaou)</option>
                    <option value="mama">Mama (Biopsia)</option>
                    <option value="pulmon">Pulmón (Biopsia)</option>
                  </select>
                </div>
                <div>
